...
class AFNetworkingDataProvider: Monstra.KVHeavyTaskBaseDataProvider<URL, Data, Progress>,
    Monstra.KVHeavyTaskDataProviderInterface
{
    ...
    private var request: AFURLSessionManager?
    ...
    func start() {
        ...
        let downloadTask = sessionManager.downloadTask(with: URLRequest(url: key), progress: { [weak self] progress in
        ...
        }, completionHandler: { [weak self] _, fileURL, error in
            guard let self = self else { return }
            ...
            if let error = error {
                ...
                self.resultPublisher(.failure(NSError(
                    domain: "AFNetworkingDataProvider",
                    code: -1,
