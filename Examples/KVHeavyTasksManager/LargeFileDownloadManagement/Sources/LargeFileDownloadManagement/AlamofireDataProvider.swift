...
class AlamofireDataProvider: Monstra.KVHeavyTaskBaseDataProvider<URL, Data, Progress>,
    Monstra.KVHeavyTaskDataProviderInterface
{
    ...
    static let resumeDataCache: MemoryCache<URL, Data> = .init(configuration: .init(
        memoryUsageLimitation: .init(memory: 1024),
        costProvider: { $0.count }
    )) // limit to 1 GB, this is a Demo project, you should modify the implementation according to your actual senario
