...
final class MockDataProvider: KVHeavyTaskBaseDataProvider<String, String, MockDataProviderProgress>,
    KVHeavyTaskDataProviderInterface
{
    ...
    private var state: State = .idle {
        didSet {
            if case let .running(value) = state {
                customEventPublisher(.init(totalLength: key.count, completedLength: value.count))
            }
        }
    }
    ...
    func start() {
        if key.contains("invalid") {
            resultPublisher(.failure(Errors.invalidKey))
            return
        }
    ...
    func stop() -> KVHeavyTaskDataProviderStopAction {
        ...
        guard case let .running(value) = state else { return .dealloc }
        state = .paused(value: value)
...
final class KVHeavyTasksManagerTests: XCTestCase {
    private func makeManager(
        priority: KVHeavyTasksManager<String, String, MockDataProviderProgress, MockDataProvider>
            .Config.PriorityStrategy,
        running: Int = 1,
        queueing: Int = 8,
        cacheConfig: MemoryCache<String, String>
            .Configuration = .defaultConfig
    )
        -> KVHeavyTasksManager<String, String, MockDataProviderProgress, MockDataProvider>
    {
    ...
    func testBasicFetchAndProgress() async {
        ...
        let minValue = events.reduce(Int.max) { partialResult, progress in
            min(partialResult, progress.completedLength)
        }
    ...
    func testConcurrentTasks() async {
        ...
        for key in ["task1", "task2", "task3"] {
            manager.fetch(key: key, result: { result in
                Task {
                    if case let .success(value) = result, let unwrappedValue = value {
    ...
    func testDataProviderReturnsNilOnStop() async {
        ...
        manager.fetch(key: "stoppable", result: { _ in
            exp2.fulfill()
        })
    ...
    func testDataProviderReturnsResumeDataOnStop() async {
        ...
        manager.fetch(key: "resumable", result: { _ in
            exp.fulfill()
        })
        ...
        manager.fetch(key: "interrupter", result: { _ in
            exp.fulfill()
        })
    ...
    func testDataCacheBehavior() async {
        ...
        manager.fetch(key: "cached", result: { _ in
            Task {
    ...
    func testResultCallbackExecutedOncePerFetch() async {
        ...
        for i in 1 ... 3 {
            manager.fetch(key: "shared", result: { _ in
                Task {
    ...
    func testResumeDataConsistency() async {
        ...
        manager.fetch(key: "longrunning", result: { _ in
            exp1.fulfill()
        })
    ...
    func testCrossScenario_LIFOAwait_Cache_MultipleCallbacks() async {
        ...
        manager.fetch(key: "cached_task", result: { _ in
            cacheExp.fulfill()
        })
    ...
    func testCrossScenario_QueueCapacity_Priority_EventOrdering() async {
        ...
        XCTAssertEqual(
            Set((finalOrder + finalEvicted).map { $0.replacingOccurrences(of: "ordered_", with: "") }),
            Set(["1", "2", "3", "4", "5"])
        )
    }
    ...
    func testCrossScenario_ErrorHandling_Resume_Events() async {
        ...
        manager.fetch(key: "error_prone", result: { _ in
            resumeExp.fulfill()
        })
    ...
    func testRapidFetchStopCycle() async {
        ...
        XCTAssertEqual(
            finalSuccessfulResults.count + finalFailedResults.count,
            cycles * 2,
            "Total results should equal operations"
        )
    ...
    func testStopAction_DeallocBehavior() async {
        let manager = makeManager(
            ...
            running: 1,
            queueing: 1000,
            cacheConfig: MemoryCache<String, String>.Configuration(memoryUsageLimitation: .init(capacity: 0, memory: 0))
        )
    ...
    func testCacheEdgeCases_NullAndInvalidKeys() async {
        ...
        let cacheConfig = MemoryCache<String, String>.Configuration(
            ...
            keyValidator: { key in
                !key.contains("invalid") // Keys with "invalid" are considered invalid
            }
        )
    ...
    func testDataProviderValidationConsistency() async {
        ...
        if case let .success(value) = finalResults["valid_direct"] {
            XCTAssertEqual(value, "valid_key_direct", "Provider should return valid key")
        } else {
            XCTFail("Valid key should succeed")
        }

        if case let .failure(error) = finalResults["invalid_direct"] {
    ...
    func testDataProviderErrorScenarios() async {
        ...
        let cacheConfig = MemoryCache<String, String>.Configuration(memoryUsageLimitation: .init(
            capacity: 0,
            memory: 0
        ))
        ...
        if case let .success(value) = finalResults["normal"] {
        ...
        if case let .failure(error) = finalResults["error"] {
    ...
    func testProviderCleanupVerification() async {
        ...
        manager.fetch(key: "cleanup_dealloc_test", result: { _ in
            Task {
                await cleanupEvents.modify { events in
        ...
        manager.fetch(key: "short_interrupt", result: { _ in
            Task {
                await cleanupEvents.modify { events in
