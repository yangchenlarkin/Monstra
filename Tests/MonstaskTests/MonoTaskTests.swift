...
final class MonoTaskTests: XCTestCase {
    ...
    func testExecuteOnceBeforeExpiration() async {
        ...
        if case let .success(value1) = result1,
           case let .success(value2) = result2,
           case let .success(value3) = result3,
           case let .success(value4) = result4
        {
    ...
    func testDifferentExecuteMethods() async {
        ...
        if let callbackRes = callbackResult,
           case let .success(callbackValue) = callbackRes,
           case let .success(asyncResultValue) = asyncResultResult,
           case let .success(asyncThrowsValue) = asyncThrowsResult
        {
    ...
    func testAsyncThrowsMethodPropagatesErrors() async {
        ...
        do {
            _ = try await task.executeThrows()
