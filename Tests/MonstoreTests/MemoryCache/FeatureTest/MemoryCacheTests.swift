...
final class MemoryCacheTests: XCTestCase {
    ...
    func testFetchResultWithComplexKeyValidator() {
        let cache = MemoryCache<Int, String>(
            configuration: .init(
                keyValidator: { key in
                    key > 0 && key <= 100 && key % 2 == 0 // Only even numbers 2-100
                }
            )
        )
    ...
    func testConfigurationWithExtremeElements() {
        let cache = MemoryCache<String, String>(
            configuration: .init(
                enableThreadSynchronization: true,
                memoryUsageLimitation: .init(capacity: 1_000_000, memory: 1_000_000), // Large but reasonable elements
                defaultTTL: .infinity,
                defaultTTLForNullElement: .infinity,
                ttlRandomizationRange: 1000.0, // Large but reasonable range
                keyValidator: { _ in true },
                costProvider: { _ in 1_000_000 } // Large but reasonable cost
            )
        )
    ...
    func testMemoryCostTrackingAfterBugFix() {
        let cache = MemoryCache<String, String>(
            configuration: .init(
                memoryUsageLimitation: .init(capacity: 10, memory: 100), // 100MB limit
                costProvider: { element in
                    element.count // Cost = string length
                }
            )
        )
    ...
    func testMemoryCostTrackingWithLargeElements() {
        let cache = MemoryCache<String, String>(
            configuration: .init(
                ...
                costProvider: { _ in 1_000_000 } // 1MB per item
            )
        )
        ...
        let cache = MemoryCache<String, String>(
            configuration: .init(
                ...
                costProvider: { element in
                    ...
                    element.count * 1024 * 1024 // 1MB per character
                }
            )
        )
