...
extension HeapTests {
    ...
    enum Operation: CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case let .insertion(value, _, _):
                return "Insert \(value)"
            case let .removal(index, _):
    ...
    class TestCase {
        ...
        func validate() -> Bool {
            ...
            heap.onEvent = {
                switch $0 {
                case let .insert(element: element, at: to):
                    element.index = to
                case .remove:
                    break
