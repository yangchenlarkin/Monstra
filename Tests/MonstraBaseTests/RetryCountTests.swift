...
final class RetryCountTests: XCTestCase {
    ...
    func testRetryCountIntegerLiteral() {
        ...
        if case let .count(count, _) = retryCount2 {
    ...
    func testRetryCountInfinityWithCustomInterval() {
        let retryCount = RetryCount.infinity(intervalProxy: .exponentialBackoff(
    ...
    func testNextTimeIntervalWithNormalScale() {
        ...
        if case let .exponentialBackoff(interval, rate) = nextProxy {
    ...
    func testNextTimeIntervalWithVeryLargeInitialInterval() {
        ...
        XCTAssertEqual(
            nextProxy.timeInterval,
            TimeInterval.greatestFiniteMagnitude,
            "Should clamp to greatestFiniteMagnitude"
        )
    }
    ...
    func testFixedIntervalDoesNotChange() {
        ...
        if case let .fixed(timeInterval) = nextProxy {
    ...
    func testExponentialBackoffBeforeFixedWithNonZeroMaxCount() {
        ...
        if case let .exponentialBackoffBeforeFixed(
            initialTimeInterval,
            originalInterval,
            maxCount,
            scaleRate
    ...
    func testExponentialBackoffAfterFixedWithZeroMaxCount() {
        ...
        if case let .exponentialBackoff(initialTimeInterval, scaleRate) = nextProxy {
            XCTAssertEqual(initialTimeInterval, 2.0, "Should scale initial interval")
            XCTAssertEqual(scaleRate, 2.0, "Should preserve scale rate")
        } else {
    ...
    func testExponentialBackoffAfterFixedWithNonZeroMaxCount() {
        ...
        if case let .exponentialBackoffAfterFixed(
            initialTimeInterval,
            originalInterval,
            maxCount,
            scaleRate
    ...
    func testExponentialBackoffBeforeFixedTransitionToFixed() {
        ...
        if case let .exponentialBackoffBeforeFixed(_, _, maxCount, _) = proxy {
        ...
        if case let .fixed(timeInterval) = proxy {
    ...
    func testExponentialBackoffAfterFixedTransitionToExponential() {
        ...
        if case let .exponentialBackoffAfterFixed(_, _, maxCount, _) = proxy {
            XCTAssertEqual(maxCount, 0, "Max count should be decremented to 0")
        } else {
        ...
        if case let .exponentialBackoff(initialTimeInterval, scaleRate) = proxy {
    ...
    func testExponentialBackoffMultipleIterations() {
        ...
        if case let .exponentialBackoff(interval, rate) = proxy {
    ...
    func testExponentialBackoffWithMaxInterval() {
        var proxy = RetryCount.IntervalProxy.exponentialBackoff(
            initialTimeInterval: TimeInterval.greatestFiniteMagnitude,
            scaleRate: 2.0
        )
        ...
        XCTAssertEqual(
            proxy.timeInterval,
            TimeInterval.greatestFiniteMagnitude,
            "Should remain at greatestFiniteMagnitude"
        )
    }
    ...
    func testIntervalProxyDefaultValues() {
        ...
        XCTAssertEqual(
            expProxy.timeInterval,
            RetryCount.IntervalProxy.DefaultInitialTimeInterval,
            "Should use default initial interval"
        )
        if case let .exponentialBackoff(_, rate) = expProxy {
            XCTAssertEqual(
                rate,
                RetryCount.IntervalProxy.DefaultExponentialBackoffScaleRate,
                "Should use default scale rate"
            )
        } else {
            XCTFail("Should be exponentialBackoff case")
        }
