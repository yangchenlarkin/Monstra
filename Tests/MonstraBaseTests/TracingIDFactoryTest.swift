...
final class TracingIDFactoryTest: XCTestCase {
    ...
    func testInitializationWithCustomLoopCount() {
        ...
        XCTAssertGreaterThanOrEqual(generatedIDs.count, Int(customLoopCount), "Should generate at least as many unique IDs as loop count allows")
    }
    ...
    func testUnsafeNextString() {
        ...
        XCTAssertTrue(safeId.rangeOfCharacter(from: CharacterSet.decimalDigits.inverted) == nil,
    ...
    func testIDUniquenessWithSmallLoopCount() {
        ...
        XCTAssertGreaterThan(uniquenessRatio, 0.2, "Should maintain reasonable uniqueness ratio even with small loop count")
        XCTAssertGreaterThanOrEqual(uniqueCount, Int(smallLoopCount), "Should generate at least as many unique IDs as loop count allows")
        
        print("Small loop count test: \(uniqueCount)/\(generationCount) unique IDs (\(String(format: "%.1f", uniquenessRatio * 100))% uniqueness)")
    }
        ...
        for threadIndex in 0..<threadCount {
            for operationIndex in 0..<(operationCount / threadCount) {
                ...
                concurrentQueue.async {
                    ...
                    XCTAssertTrue(wasInserted, "Thread \(threadIndex) operation \(operationIndex) produced duplicate ID: \(id)")
    ...
    func testUnsafeThreadSafety() {
        ...
        for threadIndex in 0..<threadCount {
            for operationIndex in 0..<(operationCount / threadCount) {
                ...
                concurrentQueue.async {
                    ...
                    XCTAssertGreaterThan(id, 0, "Unsafe method thread \(threadIndex) operation \(operationIndex) should produce positive ID")
        ...
        let idsPerSecond = Double(operationCount) / duration
        print("Unsafe thread test: \(uniqueIDs.count)/\(operationCount) unique IDs (\(String(format: "%.1f", uniquenessRatio * 100))% uniqueness) in \(String(format: "%.3f", duration))s (\(String(format: "%.0f", idsPerSecond)) IDs/sec)")
    }
    ...
    func testIDFormat() {
        ...
        for iteration in 0..<testSamples {
            ...
            XCTAssertTrue(stringID.rangeOfCharacter(from: CharacterSet.decimalDigits.inverted) == nil,
                         "String ID should be purely numeric at iteration \(iteration): '\(stringID)'")
            
            // String should be convertible to integer
            XCTAssertNotNil(Int64(stringID), "String ID should convert to Int64 at iteration \(iteration): '\(stringID)'")
        ...
        print("  String length range: \(stringLengthRange) chars (min: \(stringIDs.map(\.count).min()!), max: \(stringIDs.map(\.count).max()!))")
    }
    ...
    func testLoopCountBehavior() {
        ...
        for cycle in 0..<cycleCount {
            ...
            XCTAssertGreaterThanOrEqual(uniqueInCycle, Int(smallLoopCount), "Cycle \(cycle) should produce at least \(smallLoopCount) unique IDs")
        }
        ...
        XCTAssertGreaterThan(uniquenessRatio, 0.15, "Should maintain reasonable uniqueness across multiple counter cycles")
        XCTAssertGreaterThanOrEqual(totalUnique, Int(smallLoopCount), "Should generate at least one cycle worth of unique IDs")
        
        print("Loop count behavior: \(totalUnique)/\(totalGenerated) unique across \(cycleCount) cycles (\(String(format: "%.1f", uniquenessRatio * 100))% uniqueness)")
    }
    ...
    func testLargeLoopCount() {
        ...
        print("Large loop count test: \(ids.count) unique IDs in \(String(format: "%.3f", duration))s (\(String(format: "%.0f", generationRate)) IDs/sec)")
    }
    ...
    func testNegativeExtremeLoopCount() {
        let extremeNegativeCount: Int64 = Int64.min
    ...
    func testBaseIDCalculation() {
        ...
        print("Time-based ID calculation: Factory 1 range \(factory1Range), Factory 2 range \(factory2Range), Total unique: \(unionCount)")
    }
    ...
    func testStringConversion() {
        ...
        for iteration in 0..<conversionTestCount {
            ...
            XCTAssertGreaterThan(convertedUInt64, 0, "UInt64 to Int64 conversion should preserve positivity at iteration \(iteration)")
            ...
            XCTAssertGreaterThan(reconvertedFromString, 0, "Reconverted string should be positive at iteration \(iteration)")
            XCTAssertGreaterThan(originalUInt64, 0, "Original UInt64 should be positive at iteration \(iteration)")
            
            // String format consistency
            XCTAssertTrue(stringForm.rangeOfCharacter(from: CharacterSet.decimalDigits.inverted) == nil,
        ...
        let stringLengthRange = conversionPairs.map { $0.stringForm.count }.max()! - conversionPairs.map { $0.stringForm.count }.min()!
        let avgStringLength = Double(conversionPairs.map { $0.stringForm.count }.reduce(0, +)) / Double(conversionPairs.count)
        ...
        print("String conversion test: \(conversionTestCount) successful conversions, avg length: \(String(format: "%.1f", avgStringLength)) chars")
    }
}
